...
var maintainloop = (() => {
  ...
  let makenpcs = (() => {
    ...
    return () => {
      ...
      // Spawning
      if (c.SPAWN_CRASHER !== false) spawnCrasher(census);
      if (c.SPAWN_BOSSES !== false) spawnBosses(census);
      // Bots
      if (bots.length < c.BOTS) {
        let position = room.random();
    ...
    activate(b) {
      ...
      if (b) {
        this.intervalID = setInterval(() => {
          this.trigger();
        }, Math.max(this.interval * 1000, room.cycleSpeed));
      } else {
